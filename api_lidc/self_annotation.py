...
def parse_dir(dirname, outdir,map_uid, flatten=True, pickle=True):
    ...
    if os.path.isfile(pickle_file):
        ...
        return annotations
    annotations = parse_original_xmls(dirname, outdir, pickle,map_uid)
...
def parse_original_xmls(name,srcpath, dstpath,map_uid,pickle=True):
    ...
    annotations = []
    xml_files = find_all_files(srcpath, '.xml')
    for f in xml_files:
        annotations.append(parse(name,dstpath,f,map_uid))
...
def parse(name,dstpath,xml_filename,map_uid):
    ...
    for read_session in root.findall('nih:readingSession', NS):
        ...
        for node in nodule_nodes:
            ...
            if nodule.is_small:
                for i in range(len(nodule.rois)):
                    one_roi = nodule.rois[i]
                    z_num = map_uid[one_roi.sop_uid]
                    dst_txt = '{}/{}_{}.txt'.format(dstpath,name,z_num)
                    if os.path.isfile(dst_txt):
                        f = open(dst_txt,'a')
                    else:
                        f = open(dst_txt,'w')
                    l = one_roi.roi_xy[0][0] - 5
                    t = one_roi.roi_xy[0][1] - 5
                    r = one_roi.roi_xy[0][0] + 5
                    b = one_roi.roi_xy[0][1] + 5
                    f_str = '1 {} {} {} {}\n'.format(l,t,r,b)
                    f.writelines(f_str)
                    f.close()
                ...
                for i in range(len(nodule.rois)):
                    ...
                    if os.path.isfile(dst_txt):
                        ...
                        f = open(dst_txt,'w')
                    f_str = '0 {} {} {} {}\n'.format(one_roi.roi_rect[0],one_roi.roi_rect[1],one_roi.roi_rect[2],one_roi.roi_rect[3])
                    f.writelines(f_str)
                    f.close()
...
if __name__ == '__main__':
    parse_dir('D:/LIDC-IDRI/LIDC-IDRI/LIDC-IDRI-0001/','E:/medical_pro/LIDC-IDRI')
